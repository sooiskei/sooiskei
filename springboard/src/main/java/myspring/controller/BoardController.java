...
//			throws Exception {
	...
	public String board_cont(@RequestParam("board_num") int board_num,
		...
		BoardBean board = boardService.board_cont(board_num);	// 상세 정보 구하기(공통적으로 사용하는 부분)

		model.addAttribute("bcont", board);						// 나중에 ${bcont.필드명} 으로 출력할 수 있음
		model.addAttribute("page", page);						// 나중에 상세페이지에서 목록으로 돌아갈 때 원래 진입했던 페이지로 돌아가야하니까 페이지값을 공유
	...
	public String board_edit_ok(@ModelAttribute BoardBean b,
								@RequestParam("page") String page,
								Model model) throws Exception {
	//board_num, board_re_ref, board_re_lev, board_re_seq 의 경우, dto객체에 해당 컬럼명이 있어 modelattribute를 통해 자동으로 바인딩이 되지만
	//page는 dto객체에 해당 컬럼명이 없어서 추가로 requestparam 어노테이션으로 값을 받아줘야 함
