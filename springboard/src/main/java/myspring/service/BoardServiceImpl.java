...
public class BoardServiceImpl implements BoardService{
	...
	public void reply_ok(BoardBean b) throws Exception {
		//쿼리문을 두번 실행해야함. 왜냐하면 일단 seq 증가시켜서 새로운 댓글이 들어갈 공간을 만들어놓고, 그 이후에 삽입해야함
		boardDao.refEdit(b); // 기존 댓글 board_re_seq값 1증가

		b.setBoard_re_lev(b.getBoard_re_lev() + 1); // 부모보다 1증가된 값을 db로 전달될 dto객체인 b에 세팅함, 왜냐하면 부모글보다 한단계 더 옆쪽부터 시작해야해서
